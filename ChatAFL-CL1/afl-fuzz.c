...
void setup_llm_grammars()
{
  ...
  for (int iter = 0; iter < TEMPLATE_CONSISTENCY_COUNT; iter++)
  {
    ...
    char *grammar_output_path = alloc_printf("%s/protocol-grammars/llm-grammar-output-%d", out_dir, iter);// here i can put files locally
...
havoc_stage:
  ...
  else
  {
    ...
    stage_short = "havoc_exploit";// here!!!
...
int main(int argc, char **argv)
{
  ...
  if (protocol_selected)
  {
    protocol_patterns = kl_init(rang);  // 协议模式
    message_types_set = kh_init(strSet); // 类型集
