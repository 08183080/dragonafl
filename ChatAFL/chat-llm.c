...
char *chat_with_llm(char *prompt, char *model, int tries, float temperature)
{
    ...
    {
        url = "https://open.bigmodel.cn/api/paas/v4/"; //"https://api.openai.com/v1/completions";
    }
    else
    {
        url = "https://open.bigmodel.cn/api/paas/v4/"; //"https://api.openai.com/v1/chat/completions";
    }
    ...
    if (strcmp(model, "instruct") == 0)
    {
        asprintf(&data, "{\"model\": \"glm-4\", \"prompt\": \"%s\", \"max_tokens\": %d, \"temperature\": %f}", prompt, MAX_TOKENS, temperature);
    }
    else
    {
        asprintf(&data, "{\"model\": \"glm-4\",\"messages\": %s, \"max_tokens\": %d, \"temperature\": %f}", prompt, MAX_TOKENS, temperature);
    }
...
char *construct_prompt_stall(char *protocol_name, char *examples, char *history)  // where things are in stalling...
{
